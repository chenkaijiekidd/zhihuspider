...
class ZhihuSpider(scrapy.Spider):
    ...
    topic_follower_list_url = 'https://www.zhihu.com/api/v4/topics/{0}/followers?include=data' \
                              '%5B%2A%5D.gender%2Canswer_count%2Carticles_count%2Cfollower_count' \
                              '%2Cis_following%2Cis_followed&limit={1}&offset={2}'
    # 用户粉丝列表url
    user_follower_list_url = 'https://www.zhihu.com/api/v4/members/{0}/followers?include=data' \
                             '%5B*%5D.answer_count%2Carticles_count%2Cgender%2Cfollower_count' \
                             '%2Cis_followed%2Cis_following%2Cbadge%5B%3F(type%3Dbest_answerer)' \
                             '%5D.topics&offset={1}&limit={2}'
    # 用户详细信息url
    ...
    def parse_detail_info(self, response):
        ...
        data = json.loads(response.xpath('//script[@id="js-initialData"]//text()').extract_first())
        # 地区
        try:
